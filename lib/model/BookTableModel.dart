...
class BookTableModel {
  ...
  factory BookTableModel.fromJson(Map<String, dynamic> parsedJson) {
    int guestVal = 0;
    if (parsedJson['totalGuest'] == null ||
        parsedJson['totalGuest'] == double.nan ||
        parsedJson['totalGuest'] == double.infinity) {
      guestVal = 0;
    } else {
      if (parsedJson['totalGuest'] is String) {
        ...
        guestVal =
            (parsedJson['totalGuest'] is double)
                ? (parsedJson["totalGuest"].isNaN
                    ? 0
                    : (parsedJson['totalGuest'] as double).toInt())
    ...
    return BookTableModel(
      ...
      occasion:
          (parsedJson["occasion"] != null &&
                  parsedJson["occasion"].toString().isNotEmpty)
              ? parsedJson["occasion"]
              : "",
      specialRequest:
          (parsedJson["specialRequest"] != null &&
                  parsedJson["specialRequest"].toString().isNotEmpty)
              ? parsedJson["specialRequest"]
              : "",
      firstVisit:
          parsedJson["firstVisit"] != null ? parsedJson["firstVisit"] : false,
      totalGuest: guestVal,
    );
  }
