...
class CartProducts extends Table {
  ...
  TextColumn get packingcharges => text()();
...
class CartDatabase extends _$CartDatabase {
  ...
  addProduct(
    ...
    bool isIncerementQuantity,
  ) async {
    var joinTitleString = "";
    String mainPrice = "";
    String packingcharges = ""; // New line
    ...
    print("model.packingcharges${packingcharges}"); // Assign packingcharges
    ...
    allCartProducts.then((products) async {
      final bool _productIsInList = products.any(
        (product) =>
            product.id ==
            (model.id +
                "~" +
      ...
      if (_productIsInList) {
        CartProduct element = products.firstWhere(
          (product) =>
              product.id ==
              (model.id +
                  "~" +
        ...
        print("packingcharges Haresh${packingcharges}");
      } else {
        CartProduct entity = CartProduct(
          ...
          packingcharges: packingcharges,
          item: model.item,
          groceryWeight: model.groceryWeight,
          groceryUnit: model.groceryUnit,
          // New line
        ...
        print("packingcharges Haresh${model.groceryWeight}");
        print("packingcharges Haresh${model.groceryWeight}");
  ...
  updateProduct(CartProduct entity) =>
      (update(cartProducts)
        ..where((product) => product.id.equals(entity.id))).write(entity);
